...
final class MethodHelper extends AbstractHelper
{
    public function shouldBeSkipped(\ReflectionMethod $method, \ReflectionClass $class): bool
    {
        return $this->isFromExternalClass($method, $class)
            || str_contains($this->getModifier($method), 'private')
            || $this->isAccessor($method)
            || $this->isConstruct($method);
    }

    public function getThrowTags(\ReflectionMethod $method): array
    {
        ...
        $tags = [];
        if (preg_match_all('/@throws (.*)/', $docComment, $matches)) {
            foreach ($matches[0] as $match) {
                $tags[] = substr($match, \strlen('@throws '));
            }
        }
